...
dataset to work with. Here I'm going to load some packages, and then
run some code to gather data from the Spotify top 200 songs per week
...
(~15mins) if you run this on your computer.
...
$$\sigma$$. In simulation, we assume that we know what the values of
$$\mu$$ and $$\sigma$$ are to check what the distribution of streams
would look like if those values were true. To do that, let's write a
Stan program, which I'll save in the file `2021-12-10-streams-sim.stan`:
...
# Generalizing regression in Stan
...
multiplications of other variables. So what can we do? Well, rather
than scaling the interaction variable itself, we can actually scale
all of the variables at once in Stan by *centering* the design matrix
like so:

```{r include=FALSE, comment=''}
writeLines(readLines('2021-12-10-streams-glm-centered.stan'))
```

This last model has a few minor changes, but most of the model is the
same as before. We added a `transformed data` block, which computes
two different variables. `X_means` is a vector of the means of each
column of `X`, and `X_centered` is just `X` with the `X_means`
subtracted from each column so that each column has a mean of `0`.
