...
author: kevin
categories: [ tutorial ]
image: assets/images/2021-02-17-brms-intro/bayes_dragon_2.png
featured: true
hidden: false
output:
...
regression](dukeneuromethods.github.io). That workshop was adapted
from code provided by [Gabriela K Hajduk](gkhajduk.github.io), who in
turn referenced a workshop developed by Liam Bailey. Parts of the
tutorial are also adapted from a lesson on partial pooling by [Tristan
...
#this tweaks the sum-of-squares settings to make sure the output of Anova(model) and summary(model) are consistent and appropriate when a model has interaction terms
...
Another benefit of `brms` is that since our model is generative (i.e., <explain what you mean when you say this please>), we
can use it to simulate test scores and see how well it covers the
actual distribution of test scores. This is called a "posterior
predictive check." Here the dark line is the actual distribution of
test scores, and each light line is the distribution of test scores
predicted by a single sample of the posterior. Since the light lines
mostly cover the solid line, it looks like our model fits the data
fairly well:
...
What are we looking at here? The black points represent the mode of the posterior, the thick black lines represent the X% credible intervals, and the thinner black lines represnt the 95% credible intervals (but maybe one of those is HDIs, which I don't think you've explained yet?). The curves in gray represent the full posterior distribution. A nice thing about using a Bayesian model is that
instead of being stuck with ugly & often misleading bar charts, we can directly plot
how likely each mean is along with the full posterior distribution.
...
<insert interpretation of output that leads to the following conclusion> Looks like the effect of diet is still significant after we control for body length. We can also see that there's no significant interaction between diet and body length.
...
Here we can see that <insert interp i'm too tired to do it lol>.

<br><br><br>

Hmm.... might adding body length to our model make it better? Since we're Bayesian statisticians, we can compare *distributions* of adjusted R^^2 values for each model to answer this question:
...
As we can see, both models have an R^^2 of about 0.75, so adding body length doesn't seem to help. We can also test their difference using [LOO-IC](http://mc-stan.org/rstanarm/reference/loo.stanreg.html):
...
Okay, let's test a new question. Test scores are boring. [lame is ableist :)] I actually want to know about which dragons breathe fire. This has way more important
...
Looks like <blah blah blah, INTERPRET YOUR RESULTS PLEASE>
...
<seriously bro u gotta interpret this lol>
