...
Here we have a fairly simple shell script that demonstrates a few native functionalities.
```sh
#!/bin/sh
...
In this example I'm going to show how one can call installed programs, using [MATLAB](https://dibsmethodsmeetings.github.io/matlab-basics/) (ha!) as an example. Here we have a simple MATLAB function that generates some random figures.
```matlab
function example2
close all
clear
...
Alternatively, we can create a shell script that saves the above commands and then run that shell script. The extra step here may seem counterintuitive in this example, but you'll see how doing so can make things more convenient in more complex situations (like actual analyses).
...
Here we have a MATLAB function that takes two input parameters, subject number and session number, and saves the output or errors. 
```matlab
...
Here is the user script portion of the job script. Note that a few variables (i.e., `SUB`, `SESS`, and `EXPERIMENT`) need to be passed to this script.
```sh
...
Here is the submission shell script that loops over a number of cases (subjects and sessions) and submits those jobs to the cluster one-by-one using `qsub`. Note that the *job submission* process is serial, but the actual *jobs* are run in parallel.
```sh
#!/bin/sh
